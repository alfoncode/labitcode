...
|-------|----------|-----------|-----------|
| terreno-rustico | **2,057 KB** | **21 KB** | **-99%** |
...
That's **2.2 MB reduced to 127 KB** — a 94% total reduction.
...
For example, resizing our `terreno-rustico.webp` from its original `2560x1440` resolution to a responsive-friendly `800x450` resolution dropped its file size from **109 KB** to just **21 KB** with zero visible loss in quality. Always size your assets appropriately for the container they live in!
...
- **100/100 Mobile Lighthouse scores** in both Performance and Accessibility.
- **94% total reduction** in image payload (2.2 MB → 127 KB).
...
The best part? Most of these changes took **minutes to implement**, not hours. The image conversion and resizing was the biggest win — and it's a one-time process you can automate in your build pipeline.
